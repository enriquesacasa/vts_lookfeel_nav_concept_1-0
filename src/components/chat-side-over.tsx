...
export function ChatSideOver() {
  const { sideOverOpen, closeSideOver, pending, clearPending, openChat } = useChatPattern()
  ...
  React.useEffect(() => {
    if (!sideOverOpen || !pending) return
  ...
  return (
    <>
      {/* Backdrop */}
      {sideOverOpen && (
        <div className="fixed inset-0 z-40" onClick={closeSideOver} />
      )}

      {/* Panel */}
      <div className={cn(
        "fixed top-0 right-0 h-full z-50 flex flex-col bg-background border-l border-border shadow-2xl transition-transform duration-300 ease-in-out w-[400px]",
        sideOverOpen ? "translate-x-0" : "translate-x-full"
      )}>
        {/* Header */}
        <div className="flex items-center justify-between px-4 py-3.5 border-b border-border shrink-0">
          ...
          <div className="flex items-center gap-0.5">
            ...
            <Button variant="ghost" size="icon-sm" className="text-muted-foreground" onClick={closeSideOver}>
              <X className="h-4 w-4" />
            </Button>
          </div>
        </div>
